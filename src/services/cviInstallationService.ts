...
function toolchainKind(installation: CviInstallation): string {
...
export class CviInstallationService {
  ...
  getKnownInstallations(workspaceCviDir?: string): CviInstallation[] {
    ...
    const workspace = workspaceCviDir && fs.existsSync(workspaceCviDir) ? [this.describe(workspaceCviDir, 'workspace')].filter(hasCompiler) : [];
    const merged = [...configured, ...workspace, ...scanned];
    const byKey = new Map<string, CviInstallation>();
  ...
  getActiveInstallation(workspaceCviDir?: string, scanIfNeeded = true): CviInstallation | undefined {
    ...
    if (workspaceCviDir && fs.existsSync(workspaceCviDir)) {
      const workspace = this.describe(workspaceCviDir, 'workspace');
  ...
  async selectInstallation(workspaceCviDir?: string): Promise<CviInstallation | undefined> {
    ...
    const choices: Array<vscode.QuickPickItem & { installation?: CviInstallation; manualFolder?: boolean; manualExecutables?: boolean }> = installations.map((installation) => ({
    ...
    if (!installation.cCompilerExe && !installation.cppCompilerExe) {
      ...
      vscode.window.showWarningMessage('The selected toolchain does not expose gdb/lldb. Build and run will work, but debug may require setting labwindowsCvi.debuggerPath manually.');
    } else {
  ...
  private async promptManualExecutablePaths(): Promise<CviInstallation | undefined> {
  ...
  private async persistToolchain(installation: CviInstallation): Promise<void> {
    ...
    if (installation.cCompilerExe) {
      await this.configuration.update('cCompilerPath', installation.cCompilerExe, vscode.ConfigurationTarget.Global);
    }
    if (installation.cppCompilerExe) {
      await this.configuration.update('cppCompilerPath', installation.cppCompilerExe, vscode.ConfigurationTarget.Global);
      await this.configuration.update('intelliSenseCompilerPath', installation.cppCompilerExe, vscode.ConfigurationTarget.Global);
    } else if (installation.cCompilerExe) {
      await this.configuration.update('intelliSenseCompilerPath', installation.cCompilerExe, vscode.ConfigurationTarget.Global);
    }
    if (installation.archiverExe) {
      await this.configuration.update('archiverPath', installation.archiverExe, vscode.ConfigurationTarget.Global);
    }
    if (installation.debuggerExe) {
      await this.configuration.update('debuggerPath', installation.debuggerExe, vscode.ConfigurationTarget.Global);
    }
...
function hasCompiler(installation: CviInstallation): boolean {
