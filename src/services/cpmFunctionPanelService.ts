...
/**
 * Opens CPM .fp function-panel files without starting the CPM IDE.
 *
 * CPM function-panel resources are binary containers, but current CPM files
 ...
 */
export class CpmFunctionPanelService {
  async open(functionPanelPath: string): Promise<void> {
    ...
    const selected = await vscode.window.showQuickPick<FpQuickPickItem>(
      ...
      {
        title: `CPM Function Panel — ${path.basename(functionPanelPath)}`,
    ...
    await vscode.commands.executeCommand('cpm.library.showFunctionDetailsByName', selected.fn.name, selected.fn);
  }
}
