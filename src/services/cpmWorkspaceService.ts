...
export class CpmWorkspaceService implements vscode.Disposable {
  ...
  async moveFileToFolder(projectRef: CpmWorkspaceProjectRef, file: CpmProjectFile): Promise<void> {
    ...
    const items: Array<vscode.QuickPickItem & { targetFolder?: string; createNew?: boolean }> = [
      ...
      { label: '$(new-folder) Create new folder...', description: 'Create a CPM logical folder and move the file into it', createNew: true }
    ];

    const picked = await vscode.window.showQuickPick(items, {
      title: 'Move File To Folder',
      placeHolder: `Move ${path.basename(file.absolutePath)} to another CPM logical folder`
    });
    if (!picked) {
      return;
    }
  ...
  async moveFilesToFolder(projectRef: CpmWorkspaceProjectRef, sectionNames: string[], targetFolder: string, options: { silent?: boolean } = {}): Promise<void> {
    if (!projectRef.exists) {
    ...
    if (moved === 0) {
      if (!options.silent) {
        vscode.window.showInformationMessage('The selected file is already in this CPM logical folder.');
      }
      return;
    }
...
function collectLogicalFolders(project: CpmProject): string[] {
