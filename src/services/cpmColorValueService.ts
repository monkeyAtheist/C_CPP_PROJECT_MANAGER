...
export class CpmColorValueService {
...
function buildColorValuePickerHtml(initialHex: string, initialFormat: ColorFormat, initialAlpha: number): string {
  ...
  const formats: ColorFormatDefinition[] = [
    ...
    { value: 'hex-0x', label: '0xRRGGBB', description: 'C/C++ hexadecimal integer notation' },
    ...
    { value: 'c-rgb-list', label: 'r, g, b', description: 'Decimal channel list for C/C++ function arguments' },
...
<head>
  ...
  <style>
    ...
    .brightness-track { position: relative; width: 44px; min-height: 172px; border-radius: 999px; border: 1px solid var(--vscode-panel-border); background: linear-gradient(to top, #000000, var(--cpm-current-color, #FFFFFF)); display: flex; align-items: center; justify-content: center; padding: 8px 0; }
...
<body>
  ...
  <script>
    ...
    function renderAll() {
      ...
      brightnessTrack.style.setProperty('--cpm-current-color', fullBrightnessHex());
