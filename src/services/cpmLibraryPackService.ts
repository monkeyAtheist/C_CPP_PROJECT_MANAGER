...
function isBundledLegacyCpmPack(identity: PackIdentity | undefined, rawContent = ''): boolean {
  ...
  return id === 'cpm-structured-pack'
    || name.includes('labwindows/cpm')
    || /\b(labwindows|cpm|cpmfunc|cpmcallback|messagepopup|loadpanel|setctrlval|getctrlval|installctrlcallback)\b/i.test(haystack);
}

function quarantineLegacyCpmPackIfNeeded(targetDirectory: string, output: vscode.OutputChannel): void {
 ...
 *
 * Earlier CPM-derived builds seeded cpm_pack.json into global storage. That pack is
 * now quarantined when it is the bundled C/C++ Project Manager pack, otherwise it would keep
 * polluting the C/C++ Libraries view with CPM-only APIs.
 */
export function ensureBundledCppLibraryPack(context: vscode.ExtensionContext, output: vscode.OutputChannel): void {
  ...
  quarantineLegacyCpmPackIfNeeded(targetDirectory, output);
...
export const ensureBundledCpmLibraryPack = ensureBundledCppLibraryPack;
