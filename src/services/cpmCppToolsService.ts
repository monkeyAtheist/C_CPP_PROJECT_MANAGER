...
const CPM_CONFIGURATION_PROVIDER_ID = 'JerryCrozet-ElectronicEngineer.cpp-project-manager';
const LEGACY_CPM_CONFIGURATION_PROVIDER_IDS = ['jc-tools.labwindows-cpm-project-manager', 'JerryCrozet-ElectronicEngineer.labwindows-cpm-project-manager', CPM_CONFIGURATION_PROVIDER_ID];
...
export class CpmCppToolsService implements vscode.Disposable {
  private syncTimer: NodeJS.Timeout | undefined;
  private currentWorkspace: CpmWorkspace | undefined;
  ...
  private readonly provider: CustomConfigurationProviderLike = {
    ...
    extensionId: CPM_CONFIGURATION_PROVIDER_ID,
  ...
  requestSync(workspace: CpmWorkspace | undefined): void {
  ...
  async sync(workspace: CpmWorkspace | undefined, notify = false): Promise<string | undefined> {
    this.setCurrentWorkspace(workspace);
    if (!workspace) {
      if (notify) {
    ...
    const enabled = vscode.workspace.getConfiguration('cpm').get<boolean>('autoConfigureCppTools', true);
    if (!enabled && !notify) {
      return undefined;
    }

    const installation = this.installations.getActiveInstallation(workspace.cpmDir, notify);
    if (!installation) {
      if (notify) {
  ...
  async ensureConfigurationRootInWorkspace(workspace: CpmWorkspace | undefined = this.currentWorkspace, notify = false): Promise<boolean> {
    if (!workspace) {
      return false;
    }
    const enabled = vscode.workspace.getConfiguration('cpm').get<boolean>('autoAddCpmFolderToWorkspace', true);
    if (!enabled) {
      return false;
    }
  ...
  async addConfigurationRootToWorkspace(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<void> {
    if (!workspace) {
  ...
  async offerProviderRepairIfNeeded(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<void> {
    if (!this.hasConfiguredCpmProviderReference()) {
      return;
    }
    const action = await vscode.window.showWarningMessage(
  ...
  async autoRepairStaleProviderSelection(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<boolean> {
    ...
    const oldSetting = vscode.workspace.getConfiguration('cpm').get<boolean>('useCppToolsConfigurationProvider', false);
    if (oldSetting) {
      await vscode.workspace.getConfiguration('cpm').update('useCppToolsConfigurationProvider', false, vscode.ConfigurationTarget.Global);
  ...
  async enableAutomaticSuggestions(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<void> {
    ...
    const extensionConfig = vscode.workspace.getConfiguration('cpm');
    ...
    const action = await vscode.window.showInformationMessage(
      'Automatic C/C++ suggestions have been enabled for this workspace. Project-symbol CPM supplemental completion was disabled, while lightweight standard-library completion remains enabled. Reload VS Code; if old CPM symbols are still suggested, reset the Microsoft C/C++ IntelliSense database.',
  ...
  async repairCppToolsProviderSelection(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<void> {
  ...
  private hasConfiguredCpmProviderReference(): boolean {
    const settingName = 'default.configurationProvider';
    const inspect = vscode.workspace.getConfiguration('C_Cpp').inspect<string>(settingName);
    if (isCpmProviderId(inspect?.globalValue) || isCpmProviderId(inspect?.workspaceValue)) {
      return true;
    }
    for (const folder of vscode.workspace.workspaceFolders ?? []) {
      const scoped = vscode.workspace.getConfiguration('C_Cpp', folder.uri).inspect<string>(settingName);
      if (isCpmProviderId(scoped?.workspaceFolderValue)) {
        return true;
      }
    }
    return false;
  ...
  private async removeManagedProviderReferencesFromOpenedFolders(): Promise<number> {
    ...
    for (const folder of vscode.workspace.workspaceFolders ?? []) {
      ...
      for (const configuration of document.configurations) {
        ...
        if (isCpmProviderId(configuration.configurationProvider)) {
...
`, 'utf8');
  ...
  async diagnose(workspace: CpmWorkspace | undefined = this.currentWorkspace): Promise<void> {
    this.setCurrentWorkspace(workspace);
    ...
    const installation = this.installations.getActiveInstallation(workspace.cpmDir);
    const root = this.findConfigurationRoot(workspace.path);
    const owningFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(workspace.path));
    ...
    const message = compilerPath
      ...
      : 'No IntelliSense compiler is configured. Select a toolchain or set cpm.intelliSenseCompilerPath.';
    ...
    if (action === 'Synchronize now') {
      ...
      await vscode.commands.executeCommand('cpm.configureInstallation');
    }
  }

  private setCurrentWorkspace(workspace: CpmWorkspace | undefined): void {
  ...
  private provideConfigurations(uris: vscode.Uri[]): SourceFileConfigurationItem[] {
    ...
    const installation = this.installations.getActiveInstallation(workspace.cpmDir, false);
    if (!installation) {
      return [];
    }
    const paths = this.getProviderPaths(workspace, installation);
  ...
  private getProviderPaths(workspace: CpmWorkspace, installation: CpmInstallation): ProviderPaths {
  ...
  private collectProjectDirectories(workspace: CpmWorkspace): string[] {
  ...
  private workspaceContainsCppSources(workspace: CpmWorkspace): boolean {
    for (const projectRef of workspace.projects) {
      if (!projectRef.exists) {
        continue;
      }
  ...
  private resolveCompilerPath(installation: CpmInstallation, workspace?: CpmWorkspace): string | undefined {
    const override = vscode.workspace.getConfiguration('cpm').get<string>('intelliSenseCompilerPath', '').trim();
  ...
  private findProjectConfigurationRoot(workspace: CpmWorkspace): string | undefined {
  ...
  private createManagedConfiguration(installation: CpmInstallation, workspace: CpmWorkspace): CppToolsConfiguration {
    const compilerIncludeDirectories = findCompilerIncludeDirectories(installation);
...
function isCpmProviderId(value: unknown): boolean {
  return typeof value === 'string' && LEGACY_CPM_CONFIGURATION_PROVIDER_IDS.includes(value);
}
...
function findCompilerIncludeDirectories(installation: CpmInstallation): string[] {
...
function getToolchainBaseDirectories(installation: CpmInstallation): string[] {
