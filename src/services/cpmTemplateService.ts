 ...
 * - keeps small procedural helpers independent from the C++ CPM_Utility layer.
 *
 * @par Typical applications
 * - small C command-line tools generated by CPM;
 ...
 * - CpmUtil_ReadIniValue reads a flat key from the selected INI file.
 *
 * @par Example of use
 * @code{.c}
 * #include "{{headerFile}}"
 ...
 *
 * @details
 * This generated bundle header documents a compact, dependency-light utility
 * layer for C++ projects. It is intentionally suitable for small tools,
 * generated starters and test executables that need common helpers without a
 * larger framework.
 *
 * @par Main features
 * - loads simple INI-style configuration files;
 * - provides typed string, case, split/join and replacement helpers;
 * - retrieves the executable path/directory in a cross-platform way;
 * - provides filesystem helpers for existence checks, directory creation and
 *   text-file I/O;
 * - reads environment variables and formats local timestamps.
 *
 * @par Typical applications
 * - loading configuration next to the executable;
 * - locating resources shipped beside a generated program;
 * - creating log/output folders before writing reports;
 * - normalizing small string inputs from files, forms or command-line tools.
 *
 * @par Usage notes
 * - The executable path helpers return std::filesystem::path and require C++17.
 * - On Windows, executablePath() uses GetModuleFileNameW and handles long paths
 *   by resizing the internal buffer.
 * - Relative paths are resolved by the process current working directory unless
 *   you explicitly combine them with executableDirectory().
 *
 * @par Example of use
 * @code{.cpp}
 * #include "{{headerFile}}"
 *
 * const auto appDir = cpm::executableDirectory();
 * const auto cfgPath = appDir / "cpm_util.ini";
 * const auto logDir = appDir / "logs";
 * cpm::ensureDirectory(logDir);
 *
 * std::string value;
 * if (cpm::readIniValue(cfgPath.string(), "logPath", value))
 * {
 *     value = cpm::trim(value);
 * }
 *
 * cpm::appendLogLine((logDir / "app.log").string(), cpm::timestamp() + " started");
 * @endcode
 */
#ifndef {{guard}}
#define {{guard}}
...
std::string executableDirectoryString();
std::filesystem::path currentDirectory();
std::filesystem::path absolutePath(const std::filesystem::path &pathValue);
std::filesystem::path normalizePath(const std::filesystem::path &pathValue);
bool pathExists(const std::filesystem::path &pathValue);
bool fileExists(const std::filesystem::path &pathValue);
bool directoryExists(const std::filesystem::path &pathValue);
bool ensureDirectory(const std::filesystem::path &directoryPath);

bool readTextFile(const std::filesystem::path &filePath, std::string &contents);
bool writeTextFile(const std::filesystem::path &filePath, const std::string &contents, bool append = false);
...
std::string getEnvironmentVariable(const std::string &name, const std::string &fallback = "");
...
const CPP_CORE_UTIL_SOURCE_TEMPLATE = String.raw`/**
 * @file {{baseName}}.cpp
 * @brief Implementation of the CPM C++ core utility bundle.
 */
#include "{{headerFile}}"
...
{
/**
 * @brief Removes leading and trailing ASCII whitespace from a string copy.
...
/**
 * @brief Converts a string copy to lower case.
...
/**
 * @brief Converts a string copy to upper case.
 * @param text Input string.
...
std::vector<std::string> split(const std::string &text, char separator, bool keepEmpty)
{
    ...
    if (keepEmpty && !text.empty() && text.back() == separator)
    {
        items.emplace_back();
    }
    return items;
 ...
 * @return Formatted timestamp, or an empty string if formatting fails.
 */
...
#endif
    std::ostringstream stream;
    stream << std::put_time(&localTime, format != nullptr ? format : "%Y-%m-%d %H:%M:%S");
    return stream.str();
}
 ...
 * @throws std::runtime_error when the platform API fails.
 */
std::filesystem::path executablePath()
{
#if defined(_WIN32)
    ...
    return std::filesystem::path(std::string(buffer.data(), static_cast<std::size_t>(count)));
#endif
}
...
{
    std::error_code ec;
    auto result = std::filesystem::absolute(pathValue, ec);
...
{
    std::error_code ec;
    auto result = std::filesystem::weakly_canonical(pathValue, ec);
    if (!ec)
    {
        return result;
    }
    return absolutePath(pathValue).lexically_normal();
}

bool pathExists(const std::filesystem::path &pathValue)
{
    std::error_code ec;
    return std::filesystem::exists(pathValue, ec);
}

bool fileExists(const std::filesystem::path &pathValue)
{
    std::error_code ec;
    return std::filesystem::is_regular_file(pathValue, ec);
}

bool directoryExists(const std::filesystem::path &pathValue)
{
    std::error_code ec;
    return std::filesystem::is_directory(pathValue, ec);
}
...
bool ensureDirectory(const std::filesystem::path &directoryPath)
{
    ...
    return std::filesystem::create_directories(directoryPath, ec) || std::filesystem::is_directory(directoryPath, ec);
}
...
bool writeTextFile(const std::filesystem::path &filePath, const std::string &contents, bool append)
{
...
/**
 * @brief Reads a simple key=value pair from an INI-style file.
 * @param iniPath INI file path.
 * @param key Key to find. Section names are ignored.
 * @param value Receives the trimmed value.
...
std::string getEnvironmentVariable(const std::string &name, const std::string &fallback)
{
    ...
    DWORD required = GetEnvironmentVariableW(wideName.c_str(), nullptr, 0);
    if (required == 0)
    {
        return fallback;
    }
    std::wstring wideValue(required, L'\0');
    DWORD length = GetEnvironmentVariableW(wideName.c_str(), wideValue.data(), required);
    if (length == 0)
    {
        return fallback;
    }
    wideValue.resize(length);
    return std::filesystem::path(wideValue).string();
#else
 ...
 * @file {{headerFile}}
 * @brief CPM C++ configurable error-management API.
 *
 * @details
 * This generated bundle header documents the module directly in the file
 * copied into the user project. It summarizes the public API scope, common
 ...
 *
 * @par Typical applications
 * - C++ test executables generated by CPM;
 * - bench utilities needing a lightweight logging layer;
 * - debug builds where source-location context must be kept.
 *
 * @par Usage notes
 * - Use CPM_ERR_* macros when available so file, line and function are captured automatically.
 ...
 * cpm::loadErrorConfig("cpm_error.ini");
 * cpm::reportError(10, "Initialization failed", __FILE__, __LINE__, __func__);
 *
 * int rc = run_device_test();
 * CPM_ERR_INFZ(rc, "run_device_test failed");
 * return 0;
 *
 * error:
 * return cpm::g_errorCode;
 * @endcode
 */
#ifndef {{guard}}
#define {{guard}}
...
#define CPM_ERROR(code, message) \
    cpm::reportError((code), (message), __FILE__, __LINE__, __func__)
 ...
 * @file {{headerFile}}
 * @brief CPM C lightweight Web UI HTTP backend API.
 *
 * @details
 * This generated bundle header documents the module directly in the file
 * copied into the user project. It summarizes the public API scope, common
 ...
 * - On Windows, CPM adds ws2_32 automatically when this bundle is inserted.
 ...
 * config.port = 8080;
 * CpmWebUiServer *server = CpmWebUi_Create();
 * if (server != NULL)
 * {
 *     CpmWebUi_SetStateProvider(server, MyStateProvider, NULL);
 *     if (CpmWebUi_Start(server, &config) == 0)
 *     {
 *         CpmWebUi_Stop(server);
 *     }
 *     CpmWebUi_Destroy(&server);
 * }
 * @endcode
 */
#ifndef {{guard}}
 ...
 *
 * @par Typical applications
 * - C programs generated by CPM starters;
 ...
 * CpmError_Report(12, "Device not ready", __FILE__, __LINE__, CPM_ERROR_FUNCTION);
 * error:
 * CpmError_Log("cleanup after error code %d", g_cpmErrorCode);
 * @endcode
 */
#ifndef {{guard}}
#define {{guard}}
...
static int CpmError_FormatTimestamp(char *buffer, size_t bufferSize)
{
    time_t now;
    struct tm localTime;
...
static void CpmError_MakeSeparator(char *buffer, size_t bufferSize, char ch, size_t count)
{
    size_t i;
    if (buffer == NULL || bufferSize == 0)
        return;
...
int CpmError_LoadConfig(const char *iniPath)
{
    ...
    while (fgets(line, sizeof(line), file) != NULL)
    {
        ...
        *equals = '\0';
        ...
        else if (CPM_STRICMP(key, "mirror_to_stderr") == 0 || CPM_STRICMP(key, "mirrorToStderr") == 0)
            g_cpmErrorConfig.mirrorToStderr = CpmError_ParseBool(value, g_cpmErrorConfig.mirrorToStderr);
        else if (CPM_STRICMP(key, "max_log_lines") == 0 || CPM_STRICMP(key, "maxLogLines") == 0)
            g_cpmErrorConfig.maxLogLines = atoi(value);
        else if (CPM_STRICMP(key, "log_path") == 0 || CPM_STRICMP(key, "logPath") == 0)
                     ...
                     int line, const char *functionName)
{
    ...
    if (CpmError_FormatTimestamp(timestamp, sizeof(timestamp)) != 0)
        CpmError_CopyString(timestamp, sizeof(timestamp), "unknown time");

    CpmError_MakeSeparator(sideSep, sizeof(sideSep), '=', 26);
    snprintf(header, sizeof(header), "%s%s%s", sideSep, timestamp, sideSep);
    CpmError_MakeSeparator(fullSep, sizeof(fullSep), '=', strlen(header));
...
function getBuiltInMyUtilModules(): BundledModuleChoice[] {
  return [
    ...
    },
    {
      ...
      group: 'C bundles',
      description: 'Create cpm_lua_exec.c and cpm_lua_exec.h.',
    ...
    },
    {
      ...
      detail: 'Pure C communication stack converted from the common CPM_Utility C++ communication bundles. CAN targets Linux SocketCAN by default; I2C/SPI target Linux device files; Bluetooth RFCOMM is Windows-first.',
    ...
    {
      label: 'CPM_Utility core utilities',
      group: 'C++ bundles',
      description: 'Copy cpm_utility.cpp, cpm_utility.h and utility.ini.',
      detail: 'Canonical CPM C++ utility bundle: executable path helpers, filesystem, text I/O, INI, strings, timestamps, delays, stopwatch, matrix/vector and error-log helpers.',
      defaultFolder: 'Bundle/C++/CPM_Utility',
      entries: ['cpm_utility.cpp', 'cpm_utility.h', 'utility.ini', 'README_CPM_Utility.md=>README.md']
    },
    {
      label: 'Error management',
      group: 'C++ bundles',
    ...
    {
      label: 'CPM_Utility error management',
      group: 'C++ bundles',
      ...
      defaultFolder: 'Bundle/C++/CPM_Utility',
      entries: ['cpm_utility.cpp', 'cpm_utility.h', 'utility.ini', 'ErrorManagement/errorManagement.cpp', 'ErrorManagement/errorManagement.h']
    },
    {
      label: 'UART communication',
      group: 'C++ bundles',
    ...
    },
    {
      ...
      detail: 'Bluetooth helper from the original CPM_Utility communication set, kept separate from the full stack.',
      defaultFolder: 'Bundle/C++/CPM_Utility',
    ...
    },
    {
      ...
      detail: 'Linux I2C helper from the original CPM_Utility communication set.',
      defaultFolder: 'Bundle/C++/CPM_Utility',
    ...
    },
    {
      ...
      detail: 'Linux SPI helper from the original CPM_Utility communication set.',
      defaultFolder: 'Bundle/C++/CPM_Utility',
    ...
    },
    {
      ...
      detail: 'Generated cpm_lua_exec.c/.h bridge: launch a Lua process from C or C++, pass command-line arguments and capture output through pipes.',
      defaultFolder: 'Bundle/C++/LuaExec',
      generator: 'c-lua-bridge',
      entries: []
    },
    ...
    },
    {
      ...
      group: 'C++ bundles',
      description: 'Copy the curated CPM_Utility C++ modules.',
      detail: 'CPM_Utility, error management, communication stack, Python bridge and Web UI backend. Demo/static frontend files and OpenCV realtime demo are not included.',
      defaultFolder: 'Bundle/C++/CPM_Utility',
      entries: ['cpm_utility.cpp', 'cpm_utility.h', 'utility.ini', 'README_CPM_Utility.md=>README_CPM_Utility.md', 'ErrorManagement', 'Communication', 'external/pythonExec', 'webui/webui.cpp=>webui/webui.cpp', 'webui/webui.h=>webui/webui.h'],
      requiredWindowsLibraries: ['ws2_32']
    },
    {
    ...
    },
    {
      ...
      detail: 'Minimal Lua file showing command-line argument access and stdin/stdout exchanges with the CPM Lua execution bridge.',
...
export class CpmTemplateService {
  ...
  private async generateBundledMyUtilModule(projectDirectory: string): Promise<NewFileGenerationResult | undefined> {
    ...
    const categories = [
      ...
      { label: 'C++', description: 'Generated C++ bundles and CPM_Utility C++ modules', group: 'C++ bundles' },
