...
import { CpmBuildMode, CpmProject, CpmProjectFile, CpmRunOptions, CpmWorkspace, CpmWorkspaceProjectRef } from './types';
import { fromCpmPath, normalizeRelativePath, quote, splitCpmLongValue, toCpmPath, toCpmRuntimeStoragePath, unquote } from '../utils/pathUtils';


export interface CpmVersionInfoSettings {
...
export interface CpmSigningSettings {
  enabled: boolean;
  store: string;
  certificate: string;
  timestampUrl: string;
...
export interface CpmWorkspaceBreakpoint {
  filePath: string;
  line: number;
}

export interface CpmWorkspaceBreakpointSyncResult {
  changed: boolean;
  ...
  ignoredBreakpoints: CpmWorkspaceBreakpoint[];
  trackedBreakpoints: CpmWorkspaceBreakpoint[];
}

export interface CpmNativeTargetSettings {
  targetType: string;
  outputPath: string;
  ...
  versionInfo: CpmVersionInfoSettings;
  signing: CpmSigningSettings;
}
...
function writeText(filePath: string, content: string): void {
  ...
  if (current !== undefined && isNativeCpmDocument(filePath)) {
    createNativeBackup(filePath, current);
  }
  const temporaryPath = `${filePath}.vscode-cpm-${process.pid}-${Date.now()}.tmp`;
...
function createNativeBackup(filePath: string, content: string): string {
  const backupDirectory = path.join(path.dirname(filePath), '.vscode', 'cpm-native-backups');
...
function setPossiblyLongRuntimePathValue(section: IniSection, key: string, value: string): void {
  setPossiblyLongStringValue(section, key, toCpmRuntimeStoragePath(value));
}
...
function setPossiblyLongValue(section: IniSection, key: string, value: string): void {
  ...
  const cpmValue = toCpmPath(value);
  const chunks = splitCpmLongValue(cpmValue);
  if (chunks.length === 1) {
...
function setPossiblyLongStringValue(section: IniSection, key: string, value: string): void {
  ...
  const escapedKey = key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  section.deleteMatching(new RegExp(`^\\s*${escapedKey} Line\\d{4}\\s*=`, 'i'));
  const chunks = splitCpmLongValue(value);
  if (chunks.length === 1) {
    section.set(key, quote(chunks[0]));
    return;
  }
...
function getProjectModeSection(document: IniDocument, mode: CpmBuildMode): IniSection {
  const sectionName = `Default Build Config ${modeSuffix(mode)}`;
  const section = document.getSection(sectionName);
  if (!section) {
    throw new Error(`Invalid C/C++ project: missing [${sectionName}].`);
  }
  return section;
}
...
function resolveStoredPath(section: IniSection, key: string): string {
  ...
  return value ? fromCpmPath(value) : '';
}
...
function setOutputPath(section: IniSection, mode: CpmBuildMode, projectPath: string, value: string): void {
...
function resolveProjectFilePath(projectPath: string, section: IniSection): { absolutePath: string; relativePath?: string } {
  ...
  if (absoluteValue) {
    return { absolutePath: fromCpmPath(absoluteValue), relativePath };
  }
...
function modeSuffix(mode: CpmBuildMode): string {
  switch (mode) {
...
function createWorkspaceSccOptions(projectIndex: number): IniSection {
  ...
  section.set('Suppress CPM Error Messages', 'False');
...
function cpmWorkspaceFileType(projectFileType: string, filePath: string): string {
...
function isBreakpointCompatibleProjectFile(file: CpmProjectFile): boolean {
...
function readWorkspaceFilePath(section: IniSection): string | undefined {
  ...
  return value ? fromCpmPath(value) : undefined;
}
...
function ensureWorkspaceFileSection(
  ...
  section.set('Path', quote(toCpmPath(projectFile.absolutePath)));
  section.set('File Type', quote(cpmWorkspaceFileType(projectFile.type, projectFile.absolutePath)));
...
export class CpmParser {
  parseWorkspace(workspacePath: string): CpmWorkspace {
    const document = IniDocument.parse(readText(workspacePath));
    const header = document.getSection('Workspace Header');
    if (!header) {
      throw new Error(`Invalid C/C++ workspace: missing [Workspace Header] in ${workspacePath}`);
    }
    ...
    const projects: CpmWorkspaceProjectRef[] = [];
    ...
    return {
      ...
      activeProjectIndex,
      projects,
      cpmDir: reconstructValue(header, 'CPM Dir') ? fromCpmPath(reconstructValue(header, 'CPM Dir')!) : undefined
    };
  }

  parseStandaloneProject(projectPath: string): CpmWorkspace {
    return {
      path: projectPath,
  ...
  parseProject(projectPath: string): CpmProject {
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
      throw new Error(`Invalid C/C++ project: missing [Project Header] in ${projectPath}`);
    }

    const files = document.sections
      .filter((section) => /^File \d{4}$/i.test(section.name))
      .map((section): CpmProjectFile => {
  ...
  getNativeTargetSettings(projectPath: string, mode: CpmBuildMode): CpmNativeTargetSettings {
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
      throw new Error('Invalid C/C++ project: missing [Project Header].');
    }
  ...
  setNativeTargetSettings(projectPath: string, mode: CpmBuildMode, settings: CpmNativeTargetSettings): void {
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
      throw new Error('Invalid C/C++ project: missing [Project Header].');
  ...
  getTargetPath(projectPath: string, mode: CpmBuildMode): string | undefined {
    const document = IniDocument.parse(readText(projectPath));
    ...
    return absolute ? fromCpmPath(absolute) : undefined;
  }


  getWorkspaceRunOptions(workspacePath: string, projectIndex: number, mode: CpmBuildMode = 'debug'): CpmRunOptions {
  ...
  setWorkspaceRunOptions(workspacePath: string, projectIndex: number, mode: CpmBuildMode, options: CpmRunOptions): void {
    if (path.extname(workspacePath).toLowerCase() !== '.cws') {
      return;
    }
    const document = IniDocument.parse(readText(workspacePath));
    const header = document.getSection('Workspace Header');
    if (!header) {
      throw new Error('Invalid C/C++ workspace: missing [Workspace Header].');
    }
    ...
    if (projectIndex < 1 || projectIndex > projectCount || !header.has(`Project ${workspaceProjectSuffix(projectIndex)}`)) {
      throw new Error(`Refusing to update CPM run settings: project ${projectIndex} is not declared in the workspace.`);
    }
    ...
    if (!config) {
      throw new Error(`Unable to initialize CPM run settings: [${sectionName}] is missing.`);
    }
    // CPM persists run options per build configuration in [Default Build Config ....].
    // Do not mirror values into the legacy [Command Line Args ....] or
    // [DLL Debugging Support ....] sections: rewriting those compatibility
    // sections caused some C/C++ workspaces to become unreadable.
    ...
    for (const section of document.sections) {
      if (/^Default Build Config \d{4} (?:Debug|Release|Debug64|Release64)$/i.test(section.name) || /^DLL Debugging Support \d{4}$/i.test(section.name)) {
        for (const key of ['Working Directory', 'External Process Path']) {
          const value = reconstructValue(section, key);
          if (value && toCpmRuntimeStoragePath(value) !== value) {
            issues.push(`[${section.name}] ${key} uses a non-CPM path representation: ${value}`);
          }
        }
      }
  ...
  repairWorkspaceCompatibility(workspacePath: string): { changed: boolean; changes: string[] } {
    ...
    const normalizePathKey = (section: IniSection, key: string): void => {
      ...
      const normalized = toCpmRuntimeStoragePath(current);
  ...
  getProjectBuildActions(projectPath: string, mode: CpmBuildMode): { preBuildActions: string[]; customBuildActions: string[]; postBuildActions: string[]; nativeSectionsPresent: boolean } {
    const document = IniDocument.parse(readText(projectPath));
  ...
  setProjectBuildActions(projectPath: string, mode: CpmBuildMode, actions: { preBuildActions: string[]; customBuildActions: string[]; postBuildActions: string[] }): void {
    const document = IniDocument.parse(readText(projectPath));
    const modeName = modeSuffix(mode);
  ...
  setTargetType(projectPath: string, targetType: string): void {
    if (!['Executable', 'Dynamic Link Library', 'Static Library'].includes(targetType)) {
      throw new Error(`Unsupported CPM target type: ${targetType}`);
    }
    const document = IniDocument.parse(readText(projectPath));
    const header = document.getSection('Project Header');
    if (!header) {
  ...
  addFolderToProject(projectPath: string, folder: string): void {
    ...
    if (!normalizedFolder) {
      throw new Error('A non-empty CPM logical folder name is required.');
    }
    const document = IniDocument.parse(readText(projectPath));
  ...
  renameFolderInProject(projectPath: string, oldFolder: string, newFolder: string): void {
    ...
    if (!oldNormalized || !newNormalized) {
      throw new Error('Both CPM logical folder names are required.');
    }
  ...
  removeFolderFromProject(projectPath: string, folder: string, removeFileReferences: boolean): void {
    ...
    if (!normalized) {
      throw new Error('A CPM logical folder name is required.');
    }
  ...
  synchronizeWorkspaceBreakpoints(
    ...
    projectIndex: number,
    projectPath: string,
    requestedBreakpoints: CpmWorkspaceBreakpoint[],
    previouslyTrackedBreakpoints: CpmWorkspaceBreakpoint[] = [],
    preserveNativeBreakpoints = false
  ): CpmWorkspaceBreakpointSyncResult {
    if (path.extname(workspacePath).toLowerCase() !== '.cws') {
      throw new Error('Native CPM breakpoint synchronization requires an opened .cws workspace.');
    }
    const document = IniDocument.parse(readText(workspacePath));
    const header = document.getSection('Workspace Header');
    if (!header) {
    ...
    const projectCount = Number(header.get('Number of Projects') ?? '0');
    if (projectIndex < 1 || projectIndex > projectCount || !header.has(`Project ${workspaceProjectSuffix(projectIndex)}`)) {
      throw new Error(`Refusing to synchronize CPM breakpoints: project ${projectIndex} is not declared in the workspace.`);
    }
    ...
    const ignoredBreakpoints: CpmWorkspaceBreakpoint[] = [];
    ...
    const trackedBreakpoints: CpmWorkspaceBreakpoint[] = [];
    let appliedCount = 0;
  ...
  createWorkspaceAndProject(rootDirectory: string, workspaceName: string, projectName: string, targetType: string, cpmDir: string | undefined, formatVersion: number): { workspacePath: string; projectPath: string } {
    ...
    writeText(projectPath, this.createMinimalProjectText(projectPath, projectName, targetType, cpmDir, formatVersion));
    writeText(workspacePath, this.createMinimalWorkspaceText(workspacePath, path.basename(projectPath), cpmDir, formatVersion));
    return { workspacePath, projectPath };
  }

  createProject(projectDirectory: string, projectName: string, targetType: string, cpmDir: string | undefined, formatVersion: number): string {
    fs.mkdirSync(projectDirectory, { recursive: true });
    const projectPath = path.join(projectDirectory, `${projectName}.prj`);
    if (fs.existsSync(projectPath)) {
      throw new Error(`The C/C++ project already exists: ${projectPath}`);
    }
    writeText(projectPath, this.createMinimalProjectText(projectPath, projectName, targetType, cpmDir, formatVersion));
    return projectPath;
  }
  ...
  private requireProjectFileSection(projectPath: string, sectionName: string): { document: IniDocument; section: IniSection } {
    ...
    if (!section || !/^File \d{4}$/i.test(section.name)) {
      throw new Error(`C/C++ project file entry not found: ${sectionName}`);
    }
  ...
  private createMinimalWorkspaceText(workspacePath: string, projectFileName: string, cpmDir: string | undefined, version: number): string {
    ...
    if (cpmDir) {
      setPossiblyLongValue(header, 'CPM Dir', cpmDir);
    }
  ...
  private createMinimalProjectText(projectPath: string, projectName: string, targetType: string, cpmDir: string | undefined, version: number): string {
    const document = new IniDocument();
