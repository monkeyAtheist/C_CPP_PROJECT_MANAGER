...
import { CpmSourceSymbol, CpmSymbolService, isSourceOrHeader } from '../services/cpmSymbolService';

type FileSymbolNode = CpmSourceSymbol | { kind: 'placeholder'; label: string };

export class CpmFileSymbolsProvider implements vscode.TreeDataProvider<FileSymbolNode> {
  ...
  async reveal(symbol: CpmSourceSymbol): Promise<void> {
  ...
  getTreeItem(node: FileSymbolNode): vscode.TreeItem {
    ...
    item.command = { command: 'cpm.revealFileSymbol', title: 'Reveal C/C++ file symbol', arguments: [node] };
    return item;
  }

  async getChildren(): Promise<FileSymbolNode[]> {
    if (!this.selectedFile) {
      return [{ kind: 'placeholder', label: 'Select a C/C++ source or header file in the C/C++ Workspace.' }];
    }
