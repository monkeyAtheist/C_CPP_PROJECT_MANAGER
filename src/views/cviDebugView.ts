...
export class CviDebugView implements vscode.TreeDataProvider<CviDebugNode>, vscode.Disposable {
  ...
  getChildren(element?: CviDebugNode): CviDebugNode[] {
    ...
    if (!workspace) {
      return [
        group('Session status', [info('No workspace loaded', 'Open or create a C/C++ workspace before starting debugging.', 'info')], true, 'info'),
    ...
    const projectTooltip = ref?.exists ? ref.absolutePath : 'Select an existing project in the C/C++ Workspace view.';
    ...
    if (snapshot.execution !== 'running' && snapshot.execution !== 'suspended') {
      actions.push(action('Build & Run Debug', 'labwindowsCvi.nativeRun', 'play', 'Build locally and start a VS Code C/C++ debug session.'));
    }
    ...
    return [
      group('Session status', [
        ...
        info('Detailed stepping', 'Use the VS Code debug toolbar for stepping', 'debug-step-over'),
      ...
      group('Diagnostics', [
        info('Debug bridge', server, snapshot.serverAvailable ? 'radio-tower' : 'debug-disconnect'),
        ...
        action('Diagnose debug bridge', 'labwindowsCvi.diagnoseNativeCommandBridge', 'tools')
      ], false, 'tools')
    ];
  }
